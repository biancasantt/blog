...
Route.get('/log', async ({ view }: HttpContextContract) => {
...
Route.group(() => {
  Route.group(() => {
    ...
    Route.post('/log', 'UsersController.authlogin').as('authlogin') // Rota para exibir a página de login
    Route.post('/logout', 'UsersController.authlogout').as('authlogout')
  })
    .prefix('/users')
    .as('users')
  ...
  Route.group(() => {
    ...
    Route.get('/:id/destroy', 'PostsController.destroy').as('destroy')
  })
    .prefix('/posts')
    .as('posts')
